...
const getUserMemory = async (req, res) => {
  ...
  const { name, tag, title, description } = searchQueryValue;
  try {
...
const getAllMemory = async (req, res) => {
  ...
  try {
    ...
    if (query.length) {
      filteredMemories = await Memory.find({ $and: query }).sort({
        createdAt: -1,
      });
      if (filteredMemories.length === 0) {
        return res
      ...
      if (memories) {
        ...
        return res.status(200).send({ success: true, data: memories });
      } else {
        return res
          .status(200)
          .send({ success: false, message: "No memory", data: memories });
      }
    }
  } catch (error) {
    console.error(error);
